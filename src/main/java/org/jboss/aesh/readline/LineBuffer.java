...
public class LineBuffer {
  ...
  public LineBuffer insert(String s) {
    return insert(Parser.toCodePoints(s));
  }
  ...
  public LineStatus.Ext insertEscaped(int... codePoints) {
    ...
    Parser.consumeTo(toArray(), status);
    status.buffer.clear();
    for (int cp : codePoints) {
      if (cp < 32) {
        // Todo support \n with $'\n'
   ...
   * @return the height
   */
  public Size getCursorPosition(int width) {
   ...
   * @return the height
   */
  public Size getPosition(int offset, int width) {
    if (offset > size) {
      throw new IndexOutOfBoundsException("Offset cannot bebe greater than the buffer size");
    }
    return Parser.computePosition(data, new Size(0, 0), offset, width);
  }
  ...
  private class Update {
    ...
    public Update(Consumer<int[]> out, int width) {
      this.out = out;
      this.width = width;
      this.scrCol = getCursorPosition(width).getWidth();
      this.scrRow = getCursorPosition(width).getHeight();
    }
    ...
    public void perform(LineBuffer dst) {
      ...
      while (dstIdx < dst.size) {
        ...
        while (dstIdx < eol) {
          ...
          if (srcIdx < size && new Size(srcCol, srcRow).equals(new Size(dstCol, dstRow))) {
      ...
      moveCursor(dst.getCursorPosition(width).getWidth(), dst.getCursorPosition(width).getHeight());
