...
public class LineBuffer {
    ...
    public void replace(int pos, int cp) {
        if(pos < data.length) {
            data[pos] = cp;
        }
    }
    ...
    public void changeCase() {
        if(cursor < data.length) {
            if(Character.isUpperCase(data[cursor]))
                data[cursor] = Character.toLowerCase(data[cursor]);
            else if(Character.isLowerCase(data[cursor]))
     ...
     * @return the height
     */
    public Point getCursorPosition(int width) {
     ...
     * @return the height
     */
    public Point getPosition(int offset, int width) {
        if (offset > size) {
            throw new IndexOutOfBoundsException("Offset cannot bebe greater than the buffer size");
        }
        return Parser.computePosition(data, new Point(0, 0), offset, width);
    }
        ...
        public void perform(LineBuffer dst) {
            ...
            while (dstIdx < dst.size) {
                ...
                while (dstIdx < eol) {
                    ...
                    if (srcIdx < size && new Point(srcCol, srcRow).equals(new Point(dstCol, dstRow))) {
